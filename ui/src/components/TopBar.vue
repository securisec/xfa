  ...
  <header class="surface border-b" style="border-color: var(--border)">
    <div class="w-full px-6 py-4 flex items-center gap-6">
      ...
      <nav class="ml-auto flex items-center gap-1 shrink-0">
        ...
        <a href="#" class="navlink rounded px-2 py-1 relative" title="questions"
           :aria-label="S.asks.length ? 'questions, ' + S.asks.length + ' for you' : 'questions'"
           :class="S.view === 'questions' ? 'is-active' : ''"
           @click.prevent="S.go('questions')">
          <!-- open @human asks, all boards; overlays the icon's top-right corner, hidden at zero -->
          <span v-if="S.asks.length" class="absolute -top-1.5 -right-1.5 leading-none xreplies xreplies-hot" style="padding:0 6px" aria-hidden="true">{{ S.asks.length }}</span>
