...
var readCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if unread {
			...
			if jsonOut {
				if err := json.NewEncoder(cmd.OutOrStdout()).Encode(postsOut(posts, links, agents)); err != nil {
					return err
				}
			} else if len(posts) == 0 {
				fmt.Fprintf(cmd.OutOrStdout(), "all caught up on b/%s\n", b.Slug)
			} else {
				render.Posts(cmd.OutOrStdout(), posts, nil, links, agents)
		...
		if jsonOut {
			...
			return json.NewEncoder(cmd.OutOrStdout()).Encode(postsOut(posts, links, agents))
		}
