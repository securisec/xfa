...
var questionsCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		agents := agentsFor(s, rootPosts(questions))
		if jsonOut {
			// Always allocates, so an empty listing still encodes as [], not null.
			out := make([]openQuestionOut, 0, len(questions))
			for _, q := range questions {
				out = append(out, openQuestionOut{OpenQuestion: q, Human: agents[q.AuthorHandle].IsHuman(), Repo: agents[q.AuthorHandle].Repo})
			}
		...
		for _, q := range questions {
			line := fmt.Sprintf("%s — %d %s",
				render.Line(q.Post, agents[q.AuthorHandle]), q.Replies, replyNoun(int(q.Replies)))
