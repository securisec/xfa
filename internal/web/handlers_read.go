...
func postsJSON(posts []store.Post, human string, sess sessionIndex, links store.LinkSets, authors map[string]store.Author) []postJSON {
	out := make([]postJSON, 0, len(posts))
	for _, p := range posts {
		out = append(out, toPostJSON(p, human, sess, links, authors))
	}
	return out
}
...
func registerReadRoutes(mux *http.ServeMux, s *store.Store, human, initialBoard string) {
	...
	mux.HandleFunc("GET /api/boards/{slug}/threads", func(w http.ResponseWriter, r *http.Request) {
		...
		for _, t := range summaries {
			out = append(out, threadJSON{
				Root:         toPostJSON(t.Root, human, sess, store.LinkSets{}, authors),
				Replies:      t.Replies,
	...
	mux.HandleFunc("GET /api/boards/{slug}/board", func(w http.ResponseWriter, r *http.Request) {
		...
		for _, g := range groups {
			out = append(out, postsJSON(g, human, sess, store.LinkSets{}, authors))
		}
		writeJSON(w, http.StatusOK, out)
	})
	...
	mux.HandleFunc("GET /api/questions", func(w http.ResponseWriter, r *http.Request) {
		...
		for _, q := range questions {
			out = append(out, questionJSON{
				postJSON:        toPostJSON(q.Post, human, sess, store.LinkSets{}, authors),
				Replies:         q.Replies,
