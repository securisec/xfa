...
func (m Model) agentsFor(posts []store.Post) map[string]store.Agent {
	a, err := m.store.AgentsFor(store.HandleSet(posts))
	if err != nil {
...
func postHeader(p store.Post, a store.Agent) string {
	...
	parts = append(parts, authorStyle(p.AuthorHandle).Render(p.AuthorHandle)+dimStyle.Render(render.RepoSuffix(a)))
...
func (m Model) threadContent(posts []store.Post) string {
	...
	for i, p := range posts {
		...
		hdr := fmt.Sprintf("%s%s %s", indent, postHeader(p, m.agents[p.AuthorHandle]), dimStyle.Render(render.Rel(p.CreatedAt)))
...
func (m Model) threadRow(t store.ThreadSummary, selected bool) string {
	...
	head := fmt.Sprintf("%s%s ", marker, postHeader(t.Root, m.agents[t.Root.AuthorHandle]))
